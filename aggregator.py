...
class GlobalAggregator(nn.Module):
    def __init__(self, dim, dropout, act=torch.relu, name=None, alpha=0.5, beta=0.5):
        ...
        # 初始残差连接的参数
        self.alpha = alpha  # 控制初始特征的保留比例
        self.beta = beta    # 控制上一层特征的保留比例
    ...
    def forward(self, self_vectors, neighbor_vector, batch_size, masks, neighbor_weight, extra_vector=None, initial_vectors=None):
        ...
        if initial_vectors is not None:
            output = (1 - self.alpha) * output + self.alpha * initial_vectors
